...
class Storm:
    ...
    def __repr__(self):
        ...
        if len(idx) == 0:
            ...
            max_wind = (
                "N/A"
                if all_nan(np.array(self.dict["vmax"])[idx]) == True
                else np.nanmax(np.array(self.dict["vmax"])[idx])
            )
            min_mslp = (
                "N/A"
                if all_nan(np.array(self.dict["mslp"])[idx]) == True
                else np.nanmin(np.array(self.dict["mslp"])[idx])
            )
        summary_keys = {
        ...
        for key in self.vars.keys():
            ...
            variable_keys[
                key
            ] = f"({dtype}) [{self.vars[key][0]} .... {self.vars[key][-1]}]"
        ...
        if read_path == "" or os.path.isfile(read_path) == False:
            ...
            for key in keys:
                if key == "realtime":
                    continue
                if (
                    isinstance(self.dict[key], list) == False
                    and isinstance(self.dict[key], dict) == False
                ):
                    self[key] = self.dict[key]
                    self.coords[key] = self.dict[key]
                if (
                    isinstance(self.dict[key], list) == True
                    and isinstance(self.dict[key], dict) == False
                ):
    ...
    def sel(
        ...
        if lat is None and lon is None:
            ...
            dist = np.array(
                [
                    great_circle((lat, lon), (x, y)).kilometers
                    for x, y in zip(NEW_STORM.lon, NEW_STORM.lat)
                ]
            )
        ...
        elif (isinstance(lat, (tuple, list)) and len(lat) == 2) or (
            isinstance(lon, (tuple, list)) and len(lon) == 2
        ):
            ...
            msg = (
                "lat and lon must be of the same type: float/int, tuple/list, or None."
            )
            raise TypeError(msg)
        ...
        for key in NEW_STORM.dict.keys():
            ...
            if key == "realtime":
                continue
            if (
                isinstance(NEW_STORM.dict[key], list) == False
                and isinstance(NEW_STORM.dict[key], dict) == False
            ):
                NEW_STORM[key] = NEW_STORM.dict[key]
                NEW_STORM.coords[key] = NEW_STORM.dict[key]
            if (
                isinstance(NEW_STORM.dict[key], list) == True
                and isinstance(NEW_STORM.dict[key], dict) == False
            ):
    ...
    def interp(self, timeres=1, dt_window=24, dt_align="middle"):
        ...
        for key in NEW_STORM.dict.keys():
            if key == "realtime":
                continue
            if (
                isinstance(NEW_STORM.dict[key], (np.ndarray, list)) == False
                and isinstance(NEW_STORM.dict[key], dict) == False
            ):
                NEW_STORM[key] = NEW_STORM.dict[key]
                NEW_STORM.coords[key] = NEW_STORM.dict[key]
            if (
                isinstance(NEW_STORM.dict[key], (np.ndarray, list)) == True
                and isinstance(NEW_STORM.dict[key], dict) == False
            ):
    ...
    def to_dataframe(self, attrs_as_columns=False):
        ...
        attrs_as_columns : bool
            If True, the attributes of the storm will be added as columns in the DataFrame.
        ...
        try:
            ...
            raise RuntimeError(
                "Error: pandas is not available. Install pandas in order to use this function."
            ) from e
    ...
    def plot_nhc_forecast(
        ...
        if isinstance(forecast, int) == True:
            ...
            nhc_forecast_init_dt = [
                dt.strptime(k, "%Y%m%d%H") for k in nhc_forecast_init
            ]
            time_diff = np.array(
                [
                    (i - forecast).days + (i - forecast).seconds / 86400
                    for i in nhc_forecast_init_dt
                ]
            )
            ...
            if np.abs(time_diff[closest_idx]) >= 1.0:
                warnings.warn(
                    f"The date provided is outside of the duration of the storm. Returning the closest available NHC forecast."
                )
        else:
            raise RuntimeError(
        ...
        for k in nhc_forecast_init:
            ...
            if self.dict["year"] < 2000 and k in carq_forecast_init and 0 in hrs_carq:
                ...
                track_dict["extra_obs"].append(0) if hr in [
                    ...
                    "1500",
                    "2100",
                ] else track_dict["extra_obs"].append(1)
    ...
    def plot_gefs_ensembles(
        ...
        prop_density={
            "radius": 200,
            "cmap": plt.cm.YlOrRd,
            "levels": [i for i in range(5, 105, 5)],
        },
        domain="dynamic",
        ax=None,
        return_ax=False,
        ...
        try:
            ...
            raise RuntimeError(
                "The requested initialization isn't available for this storm."
            )
        ...
        for ens in range(0, 31):
            ...
            ds[f"gefs_{ens}"]["date"] = [
                forecast + timedelta(hours=i) for i in forecast_ens["fhr"]
            ]
            nens += 1
        ...
        for iter_fhr in range(0, 246, 6):
            ...
            for ens in range(nens):
                ...
                if iter_fhr in ds[f"gefs_{ens}"]["fhr"]:
                    ...
                    for key in ds["gfs"].keys():
                        if key not in ["date", "fhr"]:
                            temp_data[key].append(ds[f"gefs_{ens}"][key][idx])
        ...
        # Plot storm
        plot_ax = self.plot_obj.plot_ensembles(
            forecast,
            self.dict,
            fhr,
            prop_members,
    ...
    def list_nhc_discussions(self):
        ...
        if storm_year == (dt.now()).year:
            ...
            nums = "[0123456789]"
            search_pattern = (
                f"n{storm_id[0:4].lower()}{str(storm_year)[2:]}.[01]{nums}{nums}"
            )
    ...
    def get_nhc_discussion(self, forecast, save_path=None):
        ...
        if isinstance(forecast, dt) == True:
            ...
            if np.abs(closest_diff) >= 1.0:
                warnings.warn(
                    f'The date provided is unavailable or outside of the duration of the storm. Use the "list_nhc_discussions()" function to retrieve a list of available NHC discussions for this storm. Returning the closest available NHC discussion.'
                )
    ...
    def download_tcr(self, save_path=""):
        ...
        if self.source != "hurdat":
            msg = (
                "NHC data can only be accessed when HURDAT is used as the data source."
            )
            raise RuntimeError(msg)
        if self.year < 1995:
    ...
    def plot_tors(
        ...
        # Plot tornadoes
        plot_ax, leg_tor, domain = self.plot_obj_tor.plot_tornadoes(
            self.stormTors,
            domain,
            ax=ax,
            return_ax=True,
        ...
        # Plot storm
        plot_ax = self.plot_obj_tc.plot_storm(
    ...
    def plot_TCtors_rotated(self, dist_thresh=1000, return_ax=False):
        ...
        for _, row in self.stormTors.iterrows():
            plt.plot(
                [row["rot_xdist_s"], row["rot_xdist_e"] + 0.01],
                [row["rot_ydist_s"], row["rot_ydist_e"] + 0.01],
                lw=2,
                c=EFcolors[row["mag"]],
            )
        ...
        ax.plot([-dist_thresh, dist_thresh], [0, 0], "k--", lw=0.5)
        ax.plot([0, 0], [-dist_thresh, dist_thresh], "k--", lw=0.5)
        ...
        for ef, color in enumerate(EFcolors):
            ...
            handles.append(
                mlines.Line2D(
                    [], [], linestyle="-", color=color, label=f"EF-{ef} ({count})"
                )
            )
    ...
    def get_recon(
        ...
        self.recon = ReconDataset(
            self, deltap_thresh, mission_url_list, save_path, read_path, update
        )
    ...
    def get_archer(self):
        ...
        for i, line in enumerate(content):
            try:
                ...
                archer["time"].append(
                    dt.strptime(line[0] + "/" + line[1][:4], "%Y%b%d/%H%M")
                )
